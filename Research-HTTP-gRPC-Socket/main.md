...
gRPC là một framework RPC (Remote Procedure Call) mã nguồn mở do Google phát triển, sử dụng HTTP/2 và Protocol Buffers (protobuf) để truyền dữ liệu hiệu quả. gRPC là công nghệ giao tiếp máy chủ-máy khách (client-server).   
Nó được thiết kế để tối ưu hóa hiệu suất và độ tin cậy  việc giao tiếp giữa các dịch vụ phân tán, đặc biệt là trong môi trường microservices
